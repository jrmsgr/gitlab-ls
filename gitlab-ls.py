...
class GitlabIssue:
    ...
    def to_completion_item(self):
        return types.CompletionItem(label=f"#{self.id} {self.title}", kind=types.CompletionItemKind.Method if self.open else types.CompletionItemKind.Text)
...
class GitlabManager:
    ...
    def load_projects(self, projects: List[str]):
        ...
        self.ls.progress.begin(progress.token, types.WorkDoneProgressBegin(title="Database load", message="Starting progress", percentage=0))
    ...
    def update_project(self, project_name:str) -> None:
        ...
        project.issues += self.get_issue_list(project=self.client.projects.get(project.id), created_after=project.last_update)
        project.merge_requests += self.get_merge_request_list(project=self.client.projects.get(project.id), created_after=project.last_update)
    ...
    def fetch_projects(self, project_paths:List, progress:WorkProgress) -> None:
        ...
        for fetched_project in self.client.projects.list(get_all=True):
            if fetched_project.path_with_namespace in project_paths:
                ...
                self.report_progress(progress, f"Fetching missing project: {fetched_project.path_with_namespace}")
                issue_list = self.get_issue_list(fetched_project)
                merge_request_list = self.get_merge_request_list(fetched_project)
                project = GitlabProject(id=fetched_project.id, path=fetched_project.path_with_namespace, issues=issue_list, merge_requests=merge_request_list, last_update=self.get_timestamp())
                self.projects[project.path] = project
                progress.advance()
    ...
    def report_progress(self, progress: WorkProgress, msg: str):
        self.ls.progress.report(progress.token, types.WorkDoneProgressReport(message = msg, percentage = progress.progress))
    ...
    def get_issue_list(project: Project, created_after:str = None) -> List[GitlabIssue]:
        ...
        for issue in issues:
            issue_list.append(GitlabIssue(id = issue.iid, title = issue.title, author = issue.author["name"], open = (issue.state == "opened")))
    ...
    def get_merge_request_list(project: Project, created_after:str = None) -> List[GitlabMergeRequest]:
        merge_request_list = []
        logging.debug(f"Getting merge request list for project: {project.path_with_namespace} from date: {created_after}")
        if created_after is None:
        ...
        for mr in merge_requests:
            merge_request_list.append(GitlabIssue(id = mr.iid, title = mr.title, author = mr.author["name"], open = (mr.state == "opened")))
